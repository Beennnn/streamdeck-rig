...
for (const dir of ["plugin", "actions/status", "actions/slot", "actions/fixall"]) mkdirSync(`${root}/${dir}`, { recursive: true });

for (const p of ["plugin/icon", "plugin/category"]) { png(app, 28, `${root}/${p}.png`); png(app, 56, `${root}/${p}@2x.png`); }
for (const [name, svg] of [["status", status], ["slot", slot], ["fixall", fixall]]) {
